...
int searchValue1(Node* head, int val) {
    if (head == NULL) {
        printf("리스트가 비어 있습니다. 검색할 노드가 없습니다.\n");
        return -1; 
    }
    Node* current = head;
    int pos = 0; // 노드 위치를 나타내는 변수
    while (current != NULL) {
        ++pos;
        if (current->data == val) return pos; // 값이 일치하는 노드의 위치 반환
        else current = current->next; // 다음 노드로 이동
    }
    printf("값 %d를 가진 노드를 찾을 수 없습니다.\n", val);
    return -1; 
}
int searchValue2(Node* head, int val) {
    if (head == NULL) {
        printf("리스트가 비어 있습니다. 검색할 노드가 없습니다.\n");
        return -1; 
    }
...
int main() {
    ...
    for (int i = 0;i < 10;i++) {
		head = insertLast(head, i); // 0부터 9까지의 값을 리스트에 추가
    }
   
    // 리스트 출력
	printList(head);
	value = searchValue2(head, 5); // 값 6를 가진 노드 검색
	freeList(head); // 리스트 메모리 해제
